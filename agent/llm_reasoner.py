...
class GroqLlamaReasoner:
    """Routing reasoner backed by OpenRouter (name preserved for compatibility)."""
    ...
    def __init__(self, repo_root: Optional[Path] = None, token_limiter: Optional[TokenLimiter] = None) -> None:
        ...
        self.model_name = os.getenv("MODEL_NAME", "openai/gpt-4o-mini").strip() or "openai/gpt-4o-mini"
        ...
        self.openrouter_app_name = os.getenv("OPENROUTER_APP_NAME", "OracleForge").strip()
    ...
    def summarize(self, question: str, records: List[Dict[str, Any]]) -> Any:
        """Boil down retrieved records into a concise final answer for the user or benchmark."""
        if not self.openrouter_api_key:
            return records
        if not records:
            return None

        # Limit records to avoid token overflow; 20 is usually enough for a summary.
        records_json = json.dumps(records[:20], ensure_ascii=False)
        prompt = (
            f"Question: {question}\n"
            f"Retrieved Data: {records_json}\n\n"
            "Based on the retrieved data, provide a concise final answer to the question.\n"
            "- If the question asks for a count, return only the number.\n"
            "- If it asks for a name, value, or boolean, return only that result.\n"
            "- Avoid conversational filler (e.g., 'The answer is...', 'Based on the data...').\n"
            "- If you cannot find the answer, return 'No data found relevant to the query'."
        )
        ...
        try:
            ...
            body = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a precise data extraction assistant. Output ONLY the raw final value or answer requested.",
                    },
                    {"role": "user", "content": prompt},
                ],
            }
            ...
            # Basic cleanup of LLM markdown quotes if present
            content = content.strip("\"'").strip()

            # Attempt to return as numeric type if applicable
            if content.isdigit():
                return int(content)
            try:
            ...
            # Silent fallback to raw records ensures we never crash the agent loop.
            return records
